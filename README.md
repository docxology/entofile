...
**ENTO** (**EN**crypted, **T**yped, **O**mnitrack) format **0.4.0** reference implementation: a flat ZIP archive that bundles many independently typed research tracks — time series, genomics slices, spectrograms, provenance proofs — each sealed under per-track AES-256-GCM authenticated encryption, with graded observability levels and optional hash-chained proof export. The stable 0.4 paper release uses a 12-byte GCM nonce, format+track associated data, and PADMÉ length padding; opt-in format **0.5.0** adds authenticated exported-manifest context without changing the ZIP or track binary layout. Readers preserve compatibility with 0.2.0, 0.3.0, and 0.3.1 and always *verify before unpack*.
