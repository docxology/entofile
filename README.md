...
**ENTO** (**EN**crypted, **T**yped, **O**mnitrack) format **0.4.0** reference implementation: a flat ZIP archive that bundles many independently typed research tracks — time series, genomics slices, spectrograms, provenance proofs — each sealed under per-track AES-256-GCM authenticated encryption, with graded observability levels and optional hash-chained proof export. The 0.4 paper release candidate promotes the default wire contract to 0.4.0 (12-byte GCM nonce, format+track associated data, and PADMÉ length padding) while preserving read/write compatibility for 0.2.0, 0.3.0, and 0.3.1. Any ZIP tool can list a container; only the track payloads are encrypted, and readers always *verify before unpack*.
